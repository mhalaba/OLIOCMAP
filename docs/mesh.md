...
| Klucze | klucz sąsiada tylko z jego `/sync/v1/health` | zaufany sąsiad ogłasza klucze **swoich** zaufanych; zapisujemy je jako *niezaufane, poznane przez X* (TOFU, zmiana klucza = alarm w `sync_log`) |
| Przepływ wierszy | węzeł wypycha tylko własne; cudze przyjmował tylko z podpisem `relay_sig` centrali | `MESH_RELAY=1`: węzeł wypycha też cudze zweryfikowane wiersze z **podpisem origin**; odbiorca weryfikuje kluczem z katalogu — pośrednik nic nie podpisuje ani nie zmienia |
...
## Model zaufania (nie zmieniony w istocie)
...
3. **Znajomość klucza** wystarcza do weryfikacji podpisu wiersza, który przyszedł od zaufanego sąsiada. Klucz poznany „przez sąsiada” to poręczenie jednego skoku: ufasz B, B ufa C, więc wiersz C (podpisany przez C) dostarczony przez B przechodzi. Nie łączymy się z C, dopóki operator nie kliknie Zaufaj.
...
- Automatyczne podpowiadanie zaufania na podstawie liczby poręczeń (dziś: jedno poręczenie = klucz znany, zaufanie zawsze ręczne).
